...
function bindEvents() {
  ...
  els.commandHero?.addEventListener('click', (e) => { const action = e.target.closest('button')?.dataset.action; if (action === 'publish') saveAll(); if (action === 'reload') {
      window.wallboardApi.reloadScreens()
        .then((result) => result && result.ok !== false ? showSuccess('Screens restarted.') : showFailure(result?.error || 'Reload failed.'))
        .catch((err) => showFailure(`Reload failed: ${err.message}`));
    } if (action === 'identify') window.wallboardApi.identifyDisplays(); });
  els.btnSaveAll?.addEventListener('click', saveAll);
  els.btnReload?.addEventListener('click', async () => {
    try {
      const result = await window.wallboardApi.reloadScreens();
      result && result.ok === false ? showFailure(result.error || 'Reload failed.') : showSuccess('Screens restarted.');
    } catch (err) {
      showFailure(`Reload failed: ${err.message}`);
    }
  });
  ...
  els.displayGrid?.addEventListener('click', async (e) => {
    ...
    if (identify) {
      try {
        const result = window.wallboardApi.identifyDisplay
          ? await window.wallboardApi.identifyDisplay(identify.dataset.displayIdentify)
          : await window.wallboardApi.identifyDisplays();
        result && result.ok === false ? showFailure('Identify display failed.') : showSuccess('Identify display triggered.');
      } catch (err) {
        showFailure(`Identify display failed: ${err.message}`);
      }
      return;
    }
    const configure = e.target.closest('[data-display-configure]');
    const card = e.target.closest('[data-display-index]');
    if (configure) openDisplayDrawer(configure.dataset.displayConfigure);
    else if (card) openDisplayDrawer(card.dataset.displayIndex);
  });
